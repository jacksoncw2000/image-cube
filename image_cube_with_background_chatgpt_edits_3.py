...
def load_background_image_actor(img_path):
    reader = vtk.vtkPNGReader()
    reader.SetFileName(img_path)
    reader.Update()

    background_image_actor = vtk.vtkImageActor()
    background_image_actor.SetInputData(reader.GetOutput())
...
# Set the background image actor
background_actor = load_background_image_actor(background_image_path)
...
# Adjust background actor to cover the entire viewport
background_renderer.SetBackground(0, 0, 0)  # Set background to black

def resize_background_actor(renWin, event):
    size = renWin.GetSize()
    background_actor.SetScale(size[0] / background_actor.GetInput().GetDimensions()[0],
                              size[1] / background_actor.GetInput().GetDimensions()[1], 1)

render_window.AddObserver("ModifiedEvent", resize_background_actor)
